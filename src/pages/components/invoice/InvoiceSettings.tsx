...
function InvoiceSettingsModal(props: InvoiceSettingsProps) {
  ...
  useEffect(() => {
    if (userInfo && userInfo.invoiceSettings) {
      setFormData(userInfo.invoiceSettings)
    }
  }, [userInfo])
  ...
  const handleAddToTemplate = (templateType: string, option: string) => {
    ...
    if (option === "link") {
      textToAdd = `{{invoice_link}}`
    }
    if (option === "ref") {
      textToAdd = `{{invoice_ref_id}}`
    }
    if (templateType === "SMS") {
      setFormData({
        ...formData,
  ...
  return (
    <ModalBasic
      id="show-invoice-settings-modal"
      modalOpen={show}
          ...
          {selectedTab === "COMPANY" && (
            <>
              <div className="text-sm">
                Your company's info that will be shown on invoice.
              </div>
