...
export interface TableForProductItemBookingPageProps {
  ...
  handleAddFileOrImage: () => void
  fileOrImageUploading: boolean
  index: number
}
function TableForProductItemBookingPage(
  props: TableForProductItemBookingPageProps
) {
  const {
    id,
    handleChange,
        ...
        <div className="p-5 border-b border-gray-300  gap-2 flex flex-col">
          ...
          <label
            ...
            onClick={(e) => {
              e.preventDefault()
              handleAddFileOrImage()
              // getPresignURLToUpload("test.jpeg", "image/jpeg")
            }}
          >
            <img src={Gallery} alt="" />
            <div className="text-[#1C7ED6] cursor-pointer">Add new image</div>
          </label>
        </div>
      </div>
    </div>
...
export default TableForProductItemBookingPage
