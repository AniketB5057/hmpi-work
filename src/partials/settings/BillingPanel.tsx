...
function BillingPanel() {
  ...
  const fetchCustomerPortalLink = async () => {
    try {
      ...
      const getLink = await axios_instance.get(
        "/stripe-connector/customer-portal-link/main-account",
        {
          headers: {
      ...
      window.location.href = getLink.data
    } catch (err) {
      snackbar.error("failed to fetch link")
      console.log(err)
    }
  }
  ...
  if (isLoading) {
    return (
      <div className="text-center full-w">
        <LoadingSpinner />
      </div>
    )
  }
